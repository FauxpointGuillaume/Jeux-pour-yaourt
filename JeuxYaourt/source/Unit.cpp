...
void Unit::moveunit(int destinationX, int destinationY)
{
	if (reset_action)
	{
		...
		if (move == true)
		{
			...
			if (mousexy[0] <= sprite->GetX())
			{
				distancecube2x = sprite->GetX() - mousexy[0];
				for (i = 0; i < distancecube2x; ++i)
				{
					movingx.push_back(sprite->GetX() - i);
				}
			}
			else if (mousexy[0] > sprite->GetX())
			{
				distancecube2x = mousexy[0] - sprite->GetX();
				for (i = 0; i < distancecube2x; ++i)
				{
					movingx.push_back(sprite->GetX() + i);
				}
			}
			if (mousexy[1] <= sprite->GetY())
			{
				distancecube2y = sprite->GetY() - mousexy[1];
				for (i = 0; i < distancecube2y; ++i)
				{
					movingy.push_back(sprite->GetY() - i);
				}
			}
			else if (mousexy[1] > sprite->GetY())
			{
				distancecube2y = mousexy[1] - sprite->GetY();
				for (i = 0; i < distancecube2y; ++i)
				{
					movingy.push_back(sprite->GetY() + i);
				}
...
void Unit::draw()
{
	...
	if (anim = true)
	{
		sprite->PlayAnimation(anim_beginframe,anim_endframe,animrow_to_use,anim_speed);
	}
}
...
void Unit::anim_nade()
{
	...
	if (anim_nade_start)
	{
		if (animationdelay + animationtime < SDL_GetTicks())
		{
