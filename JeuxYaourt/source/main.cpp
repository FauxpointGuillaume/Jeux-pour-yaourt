...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			//	}
				...
				for (int y = bob->getsprite()->GetY(); y < bob->getsprite()->GetY() + 100; ++y)
				{
					++loly;
					for (int x = bob->getsprite()->GetX(); x < bob->getsprite()->GetX() + 100; ++x)
					{
...
/////////////////////////////////////////Fin du brouillon 3 du fogofwar
				...
				for (auto &ally : allyunits)
				{
					...
					if (ally->getselect())
					{
						if (ally->action_size() > 0)
						{
							...
							{
								destination_circle->SetX(ally->getmousex());
								destination_circle->SetY(ally->getmousey());
							...
							for (auto &action : ally->action_getlist())
							{
								if (action == 1)
								{
									destination_circle->SetX(ally->mousex_at(i));
									destination_circle->SetY(ally->mousey_at(i));
									destination_circle->Draw();
								}
								else if (action >= 5)
								{
									destination_circle_skills->SetX(ally->mousex_at(i));
									destination_circle_skills->SetY(ally->mousey_at(i));
				...
				destination_circle->PlayAnimation(0, 7, 0, 100);
				destination_circle_skills->PlayAnimation(0, 7, 0, 100);

				for (auto &aze : units) // access by reference to avoid copying
				{
					if (aze->getpre_select() || aze->getselect())
					{
	...
	//Free global font
	TTF_CloseFont(gFPSTextTexture.gFont);
